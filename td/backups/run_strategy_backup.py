...
def run_module(package: str, name: str):
    ...
    If 'package' is empty: import module by name only (direct file from sys.path)
    Otherwise: import td.<package>.<name>
    """
    if package:
        full_path = f"td.{package}.{name.lower()}"
    else:
        full_path = name.lower()   # Always define full_path

    try:
        module = importlib.import_module(full_path)
    except Exception as e:
        raise RuntimeError(f"Could not load module {full_path}: {e}") from e

    if not hasattr(module, "main"):
        raise RuntimeError(f"Module {full_path} has no main() function")
...
def main():
    ...
    # ----------------------------------------
    if args.news:
        module_path = Path(__file__).resolve().parent.parent / "news"
        sys.path.insert(0, str(module_path))
        log.info("Running news module: %s", args.news)
        run_module("", args.news)   # empty package → direct file import
    ...
    # ----------------------------------------
    if args.update:
        module_path = Path(__file__).resolve().parent.parent / "core" / "update"
        sys.path.insert(0, str(module_path))
        log.info("Running update module: %s", args.update)
        run_module("", args.update)  # empty package → direct file import
